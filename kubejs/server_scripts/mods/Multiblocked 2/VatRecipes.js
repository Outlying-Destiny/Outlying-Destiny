ServerEvents.recipes((event) => {
    ...
    function vat(outputf, outputfcount, inputf, inputfcount, inputi1, inputi2, time, id){event.recipes.kubejs.the_vat().id("kubejs:the_vat/"+id).inputItems(inputi1, inputi2).inputFluids(inputf+' '+inputfcount).outputFluids(outputf+' '+outputfcount).perTick(vat => vat.inputFE(128)).duration(time)}
    ...
    nutrient.forEach((nutrient) => {
        vat('enderio:nutrient_distillation', 250*nutrient.mult, 'minecraft:water', 1000*nutrient.mult, nutrient.input, 'minecraft:sugar', 180, 'nutrient_sugar_'+nutrient.mult)
        vat('enderio:nutrient_distillation', 375*nutrient.mult, 'minecraft:water', 1500*nutrient.mult, nutrient.input, '#forge:mushrooms', 180, 'nutrient_mushrooms_'+nutrient.mult)
        vat('enderio:nutrient_distillation', 500*nutrient.mult, 'minecraft:water', 2000*nutrient.mult, nutrient.input, 'minecraft:fermented_spider_eye', 180, 'nutrient_fermented_spider_eye_'+nutrient.mult)
    })
